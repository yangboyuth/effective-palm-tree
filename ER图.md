...
| 符号 | 含义 | 示例（本项目） | 说明 |
|------|------|------|------|
| `\|\|--\|\|` | **一对一 (1:1)** | 用户表 \|\|--\|\| 用户详情表 | 用户详情表共享用户编号作主键+外键，一用户对应一条详情 |
| `\|\|--o{` | **一对多 (1:N)** | 用户表 \|\|--o{ 收货地址表 | 收货地址表外键为用户编号，一用户可有多个地址 |

> 本项目所有 34 条外键关系中，1:1 关系仅 4 处，其余 30 处均为 1:N 关系，无多对多。
...
| 缩写 | 全称 | 含义 | 示例 |
|------|------|------|------|
| **PK** | Primary Key | **主键**，唯一标识一条记录，不允许为空 | 用户编号(PK) |
| **FK** | Foreign Key | **外键**，引用另一张表的主键，建立表间关联 | 用户编号(FK) — 引用用户表 |
| **UK** | Unique Key | **唯一约束**，字段值在表中不可重复 | 订单流水号(UK)、用户名(UK) |
...
|------|------|
| `1:1` | 一对一关系 |
| `1:N` | 一对多关系（父表一条对应子表多条） |
| `N:1` | 多对一关系（子表多条对应父表一条，即外键关系） |
...
```mermaid
gantt
    title 淘宝商城数据库课程设计计划进度
    dateFormat  YYYY-MM-DD
    axisFormat  %m-%d
    tickInterval 1day

    section 第一阶段 需求分析
    需求分析与概念设计           :done, a1, 2024-06-03, 2d

    section 第二阶段 结构设计
    逻辑结构设计与ER图绘制       :done, a2, after a1, 2d

    section 第三阶段 物理实现
    建表脚本编写                 :done, a3, after a2, 1d
    测试数据构造与验证           :done, a4, after a3, 1d

    section 第四阶段 文档完善
    文档撰写                     :active, a5, after a4, 1d
    修改完善与提交               :a6, after a5, 1d
```
...
| 阶段 | 任务内容 | 工期 | 交付物 |
|------|----------|------|--------|
| 第一阶段 需求分析 | 分析淘宝电商业务流程，确定五大模块，识别核心实体及属性，划分 25 张表结构 | 2天 | 需求分析说明、实体关系草图 |
| 第二阶段 结构设计 | 使用 ProcessOn 绘制 ER 图，定义每张表的字段、主键、外键和约束，标注关系类型 | 2天 | 完整 ER 图（整体图 + 分模块图） |
| 第三阶段 物理实现 | 在 Navicat 中编写建表 DDL 脚本，插入 281 条关联测试数据，上机验证数据完整性 | 2天 | 建表.sql、测试数据.sql |
| 第四阶段 文档完善 | 撰写课程设计文档（目的要求、选题意义、可行性分析、数据字典），修改完善并提交 | 2天 | 完整课程设计报告 |
